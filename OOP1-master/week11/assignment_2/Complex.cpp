...
Complex::Complex(float _val1, float _val2) {
    /**
     * std 네임 스페이스의 make_pair를 통해 pair 객체 생성 가능
     * real()과 imag에 각각 _val1, _val2를 대입
     */
...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		this->set_val(this->real() + a->val() , this->imag());
		delete(a);
		return this;
	}
		...
		a->set_val(a->real() + this->real(), a->imag());
		delete(this);
		return a;
	}
	else {
...
Number *Complex::sub(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		Complex* a = dynamic_cast<Complex*>(num);
		a->set_val(this->real() - a->real(), a->imag());
		delete(this);
		return a;
	}
	else {
...
Number *Complex::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* a = dynamic_cast<Integer*>(num);
		this->set_val(this->real() * a->val() , this->imag() * a->val());
		delete(a);
		return this;
	}
	else if (num->types() == FLOAT) {
		...
		Complex* a = dynamic_cast<Complex*>(num);
		a->set_val((this->real() * a->real()) - (this->imag() * a->imag()), (this->real() * a->imag()) + (this->imag() * a->real()));
		delete(this);
		return a;
	}
	else {
...
void Complex::set_val(double real, double imag) {
     ...
     * 이 때 make_pair함수를 이용
     */
	std::pair<double, double> newValue = std::make_pair(real, imag);
	value = newValue;
}
