...
int main() {
	...
	int m4 = 0;
	if(!m.empty()){
		 m1 = m["first"]; // m에서 key값을 넣어 value를 얻어 채우세요
		 m2 = m["second"]; // m에서 key값을 넣어 value를 얻어 채우세요
		 m3 = m["third"]; // m에서 key값을 넣어 value를 얻어 채우세요
		 m4 = m["forth"]; // m에서 key값을 넣어 value를 얻어 채우세요
	}
